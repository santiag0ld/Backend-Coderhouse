...
import { productsService, userService, cartService} from '../repository/service.js';
import { logger } from "../utils/logger.js";
import fetch from "node-fetch";

class ViewsController {
  constructor() {
    this.productViewService = productsService;
  ...
  products = async (req, res) => {
    try {
      ...
      const apiUrl = new URL(
        `http://localhost:${configObject.port}/api/products`
      );
  ...
  cart = async (req, res) => {
    try {
      ...
      let resp = await fetch(
        `http://localhost:${configObject.port}/api/carts/${cart}`
      );
